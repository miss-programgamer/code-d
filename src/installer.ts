...
export default class Installer {
	...
	determineOutputFolder(): string {
		if (process.platform == "linux" && process.env.HOME) {
			if (fs.existsSync(path.join(process.env.HOME, ".local", "share"))) {
				return path.join(process.env.HOME, ".local", "share", "dlang", "bin");
			} else {
				return path.join(process.env.HOME, ".dlang", "bin");
			}
		} else if (process.platform == "win32" && process.env.APPDATA) {
			return path.join(process.env.APPDATA, "dlang", "bin");
		} else {
			return path.join(extension.path, "bin");
		}
	}
	...
	async fetchNightlyRelease(timeout: number): Promise<Release | undefined> {
		...
		let targetAsset = this.findFirstMatchingAsset("nightly", assets);
		let ret: Release = {
			name: "nightly",
			asset: targetAsset
		};
	...
	async fetchLatestTaggedRelease(channel: "stable" | "beta", timeout: number): Promise<Release | undefined> {
		...
		for (let i = 0; i < body.length; i++) {
			...
			let targetAsset = this.findFirstMatchingAsset(release.tag_name, release.assets);
			if (!targetAsset) {
				if (ret.name == "master") {
	...
	updateAndInstallServeD(env: any): Thenable<boolean | undefined | "retry"> {
		return window.withProgress({
			...
			if (version === undefined) {
				...
				return window.showInformationMessage("Updates can currently not be determined. Would you like "
	...
	installServeD(urls: { url: string, title: string; }[], ref: string): (env: NodeJS.ProcessEnv) => Promise<boolean | undefined | "retry"> {
		if (urls.length == 0) {
			return async (env: any) => {
				const r = await window.showErrorMessage("No precompiled serve-d binary for this platform/architecture", "Compile from source");
